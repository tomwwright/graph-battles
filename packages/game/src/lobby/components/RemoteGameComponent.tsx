...
const openGameUrl = (gameId: string, userId: string) => {
  const url = `/assets/html/index.html?gameId=${gameId}&userId=${userId}`;
  window.open(url, '_blank');
};

export const RemoteGameComponent: React.StatelessComponent<RemoteGameProps> = ({ game, userId }) => {
  return (
    <Card my={1}>
      <Flex p={2}>
        <Box width={1 / 4} m={1}>
          <Subhead>{game.gameId}</Subhead>
          <Small>
        ...
        <Box width={1 / 2} m={1}>
          ...
          <Text pt={2}>
            <Small>{toLeaderboardText(game.leaderboard)}</Small>
          </Text>
        </Box>
