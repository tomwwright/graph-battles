      ...
      <Flex p={2}>
        <Box width={1 / 4} m={1}>
          ...
          <Small>
            ...
            <PlayerList users={game.gameData.users} players={game.gameData.users.map(user => game.gameData.maps[0].dataMap[user.playerIds[0]] as PlayerData)} />
          </Small>
        </Box>
        <Box width={1 / 2} m={1}>
          <Text>Turn {game.gameData.maps.length}/{game.gameData.maxTurns}</Text>
          <Text>Victory Points {leaders[0].victoryPoints}/{game.gameData.maxVictoryPoints}</Text>

          <Text pt={2} ><Small>Updated {toTimeDescription(Date.now() - game.lastUpdated)} ago</Small></Text>
        </Box>
