import { GameMapData, PendingActionType } from '../map';
import { Status, TerritoryType, TerritoryProperty, TerritoryAction, propsToType, propsToActions } from '../values';

const testMap: GameMapData = {
  id: 'test-map',
  type: 'map',
  nextId: 0,
  pendingActions: [
    { type: PendingActionType.MOVE, unitId: '#UR3', destinationId: '#T2' },
    { type: PendingActionType.MOVE, unitId: '#UR4', destinationId: '#T3' },
    { type: PendingActionType.MOVE, unitId: '#UB1', destinationId: '#T2' },
    { type: PendingActionType.MOVE, unitId: '#UB2', destinationId: '#T2' },
    { type: PendingActionType.MOVE, unitId: '#UB4', destinationId: '#T4' },
    { type: PendingActionType.MOVE, unitId: '#UG1', destinationId: '#T4' },
    { type: PendingActionType.TERRITORY, territoryId: '#T1', action: TerritoryAction.CREATE_UNIT },
    { type: PendingActionType.TERRITORY, territoryId: '#T2', action: TerritoryAction.CREATE_UNIT },
    { type: PendingActionType.TERRITORY, territoryId: '#T3', action: TerritoryAction.BUILD_SETTLEMENT },
  ],
  dataMap: {
    '#PR': {
      id: '#PR',
      type: 'player',
      colour: 16711680,
      gold: 5,
      goldProduction: 0,
    },
    '#PB': {
      id: '#PB',
      type: 'player',
      colour: 255,
      gold: 1,
      goldProduction: 1,
    },
    '#PG': {
      id: '#PG',
      type: 'player',
      colour: 10027263,
      gold: 10,
      goldProduction: 2,
    },
    '#T1': {
      id: '#T1',
      type: 'territory',
      playerId: '#PB',
      edgeIds: ['#E12', '#E13'],
      food: 0,
      properties: [
        TerritoryProperty.SETTLED,
        TerritoryProperty.FARM,
        TerritoryProperty.FORT,
        TerritoryProperty.CITY,
        TerritoryProperty.CASTLE,
      ],
    },
    '#T2': {
      id: '#T2',
      type: 'territory',
      playerId: '#PR',
      edgeIds: ['#E12', '#E23'],
      food: 2,
      properties: [],
    },
    '#T3': {
      id: '#T3',
      type: 'territory',
      playerId: '#PB',
      edgeIds: ['#E13', '#E23', '#E34', '#E35'],
      food: 3,
      properties: [],
    },
    '#T4': {
      id: '#T4',
      type: 'territory',
      playerId: null,
      edgeIds: ['#E34', '#E45'],
      food: 3,
      properties: [TerritoryProperty.SETTLED, TerritoryProperty.CITY],
    },
    '#T5': {
      id: '#T5',
      type: 'territory',
      playerId: '#PG',
      edgeIds: ['#E35', '#E45'],
      food: 6,
      properties: [TerritoryProperty.SETTLED, TerritoryProperty.FARM],
    },
    '#E12': {
      id: '#E12',
      type: 'edge',
      territoryAId: '#T1',
      territoryBId: '#T2',
    },
    '#E13': {
      id: '#E13',
      type: 'edge',
      territoryAId: '#T1',
      territoryBId: '#T3',
    },
    '#E23': {
      id: '#E23',
      type: 'edge',
      territoryAId: '#T2',
      territoryBId: '#T3',
    },
    '#E34': {
      id: '#E34',
      type: 'edge',
      territoryAId: '#T3',
      territoryBId: '#T4',
    },
    '#E35': {
      id: '#E35',
      type: 'edge',
      territoryAId: '#T3',
      territoryBId: '#T5',
    },
    '#E45': {
      id: '#E45',
      type: 'edge',
      territoryAId: '#T4',
      territoryBId: '#T5',
    },
    '#UR1': {
      id: '#UR1',
      type: 'unit',
      playerId: '#PR',
      locationId: '#T1',
      statuses: [],
    },
    '#UR2': {
      id: '#UR2',
      type: 'unit',
      playerId: '#PR',
      locationId: '#T1',
      statuses: [],
    },
    '#UR3': {
      id: '#UR3',
      type: 'unit',
      playerId: '#PR',
      locationId: '#T1',
      statuses: [],
    },
    '#UR4': {
      id: '#UR4',
      type: 'unit',
      playerId: '#PR',
      locationId: '#T2',
      statuses: [Status.DEFEND],
    },
    '#UB1': {
      id: '#UB1',
      type: 'unit',
      playerId: '#PB',
      locationId: '#T3',
      statuses: [],
    },
    '#UB2': {
      id: '#UB2',
      type: 'unit',
      playerId: '#PB',
      locationId: '#T3',
      statuses: [],
    },
    '#UB3': {
      id: '#UB3',
      type: 'unit',
      playerId: '#PB',
      locationId: '#T3',
      statuses: [Status.STARVE],
    },
    '#UB4': {
      id: '#UB4',
      type: 'unit',
      playerId: '#PB',
      locationId: '#T3',
      statuses: [Status.STARVE],
    },
    '#UG1': {
      id: '#UG1',
      type: 'unit',
      playerId: '#PG',
      locationId: '#T5',
      statuses: [],
    },
    '#U1': {
      id: '#U1',
      type: 'unit',
      playerId: null,
      locationId: '#T4',
      statuses: [Status.DEFEND],
    },
  },
};

export default testMap;
