...
/** Exhaust a generator, discarding yielded values */
function exhaust<T>(gen: Generator<T>): void {
  while (!gen.next().done) {}
}
...
describe('Resolver', () => {
  ...
  it('resolve adding defend status', () => {
    ...
    exhaust(resolveMoves(resolved));
    exhaust(resolveAddDefendStatus(resolved, map));
  ...
  it('resolve territory actions', () => {
    expect(map.territory('#T1').currentAction).to.equal(
    ...
    exhaust(resolveTerritoryActions(map));

    expect(map.territory('#T1').currentAction).to.equal(null, 'Territory #T1 post-resolve current action');
    expect(map.territory('#T1').units).to.have.length(
      3,
      '#T1 still has 3 units because opposing units stifled the Create Unit'
    );
    expect(map.territory('#T2').currentAction).to.equal(null, '#T2 post-resolve current action is cleared');
    expect(map.territory('#T2').units).to.have.length(2, '#T2 has a new unit created by Create Unit');
    expect(map.territory('#T3').currentAction).to.equal(null, 'Territory #T3 post-resolve current action');
    expect(map.territory('#T3').hasProperty(TerritoryProperty.SETTLED)).to.equal(
      true,
  ...
  it('resolve territory control', () => {
    ...
    expect(preresolveMap.territory('#T1').currentAction).to.equal(
      TerritoryAction.CREATE_UNIT,
      'Territory #T1 initial current action'
    );
    expect(preresolveMap.territory('#T2').currentAction).to.equal(
      TerritoryAction.CREATE_UNIT,
      'Territory #T2 initial current action'
    );
    expect(preresolveMap.territory('#T3').currentAction).to.equal(
      TerritoryAction.BUILD_SETTLEMENT,
      'Territory #T3 initial current action'
    );
    expect(preresolveMap.territory('#T4').currentAction).to.equal(null, 'Territory #T4 initial current action');
    expect(preresolveMap.territory('#T5').currentAction).to.equal(null, 'Territory #T5 initial current action');

    exhaust(resolveMoves(map));
    exhaust(resolveTerritoryControl(map, preresolveMap));
    ...
    expect(map.territory('#T4').currentAction).to.equal(null, 'Territory #T4 post-resolve current action');
    expect(map.territory('#T5').currentAction).to.equal(null, 'Territory #T5 post-resolve current action');
  });
});
