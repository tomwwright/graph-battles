...
describe('Territory Model', () => {
    ...
    for (const territory of map.territories) {
      if (territory.data.playerId)
        expect(territory.player.data.id).to.be.equal(
          ...
          `Territory ${territory.data.id}: player getter`
        );
      else expect(territory.player).to.be.null;
  ...
  it('territory properties', () => {
    ...
    expect(territory.data.properties).to.have.members(
      [TerritoryProperty.SETTLED, TerritoryProperty.CITY],
      'starting territory properties'
    );
    territory.addProperty(TerritoryProperty.SETTLED);
    expect(territory.data.properties).to.have.members(
      [TerritoryProperty.SETTLED, TerritoryProperty.CITY],
      'add existing settled property: no change'
    );
    territory.removeProperty(TerritoryProperty.FARM);
    expect(territory.data.properties).to.have.members(
      [TerritoryProperty.SETTLED, TerritoryProperty.CITY],
      'remove non-existing farm property: no change'
    );
    territory.addProperty(TerritoryProperty.FARM);
    expect(territory.data.properties).to.have.members(
      [TerritoryProperty.SETTLED, TerritoryProperty.CITY, TerritoryProperty.FARM],
      'add farm property: farm added'
    );
    territory.removeProperty(TerritoryProperty.CITY);
    expect(territory.data.properties).to.have.members(
      [TerritoryProperty.SETTLED, TerritoryProperty.FARM],
