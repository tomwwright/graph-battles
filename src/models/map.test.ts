import { SELECTED_ALPHA } from 'game/constants';
import GameMap, { GameMapData } from 'models/map';
import { Status, TerritoryAction, TerritoryType, TerritoryProperty } from 'models/values';
...
describe('Map Model', () => {
  ...
  it('getters', () => {
    ...
    for (const unitId of map.data.unitIds)
      expect(map.unit(unitId).data.id).to.equal(unitId);
    for (const territoryId of map.data.territoryIds)
      expect(map.territory(territoryId).data.id).to.equal(territoryId);
    for (const edgeId of map.data.edgeIds)
      expect(map.edge(edgeId).data.id).to.equal(edgeId);
    for (const playerId of map.data.playerIds)
      expect(map.player(playerId).data.id).to.equal(playerId);
  ...
  it('add unit', () => {
    ...
    expect(map.unit("#1").data.playerId).to.equal(map.territory("#T1").data.playerId, 'unit added to territory has same controlling player');
  ...
  it('resolve food', () => {
    ...
    expect(map.territory("#T1").data.food).to.equal(0, 'Territory #T1 has correct resolved food: caps at zero, units starve');
  ...
  it('resolve territory actions', () => {
    expect(map.territory("#T1").data.currentAction).to.equal(TerritoryAction.CREATE_UNIT, "Territory #T3 initial current action");
    expect(map.territory("#T1").data.unitIds).to.have.length(3, "Territory #T1 initial number of units");
    expect(map.territory("#T3").data.currentAction).to.equal(TerritoryAction.BUILD_SETTLEMENT, "Territory #T3 initial current action");
    expect(map.territory("#T3").hasProperty(TerritoryProperty.SETTLED)).to.equal(false, "Territory #T3 settled property");
    ...
    expect(map.territory("#T3").hasProperty(TerritoryProperty.SETTLED)).to.equal(true, "Territory #T3 settled property post-resolve");
  ...
  it('resolve territory control', () => {
    ...
    expect(preresolveMap.territory("#T1").data.currentAction).to.equal(TerritoryAction.CREATE_UNIT, "Territory #T1 initial current action");
    expect(preresolveMap.territory("#T2").data.currentAction).to.equal(null, "Territory #T2 initial current action");
    expect(preresolveMap.territory("#T3").data.currentAction).to.equal(TerritoryAction.BUILD_SETTLEMENT, "Territory #T3 initial current action");
    ...
    expect(map.territory("#T3").data.currentAction).to.equal(TerritoryAction.BUILD_SETTLEMENT, "Territory #T3 post-resolve current action");
